...
const WORKFLOW_ID = 'c88d84d9-6883-4279-b55b-4966054a097e';
...
test('shipped markup carries workflow markers', () => {
  ...
  assert.match(html, /Web版多用户同屏贪吃蛇大战 · postdeploy 4046dbc8/);
  assert.match(html, new RegExp(WORKFLOW_ID));
  assert.match(html, /<title>Web版多用户同屏贪吃蛇大战 · postdeploy 4046dbc8<\/title>/);
  assert.match(html, /<h1 class="title">Web版多用户同屏贪吃蛇大战 · postdeploy 4046dbc8<\/h1>/);
});
