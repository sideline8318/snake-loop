...
function headCell(dom, player = 'player1') {
  const canvas = dom.window.document.getElementById('gameCanvas');
  const colors = player === 'player2' ? ['#ffcf70'] : undefined;
  const r = lastHeadDraw(canvas._ctxLog, colors);
  if (!r) return null;
...
test('screen direction buttons steer player one', async () => {
  ...
  try {
    ...
    await waitFor(() => headCell(dom));
    const before = headCell(dom);
    d.querySelector('[data-direction="up"]').dispatchEvent(
...
test('WASD keys steer correctly [regression: bug A fixed]', async () => {
  ...
  for (const { key, expect } of cases) {
      ...
      try {
        ...
        const before = headCell(dom, 'player2');
        press(dom, key);
        const after = await waitFor(() => {
          const c = headCell(dom, 'player2');
          return c && (c.x !== before.x || c.y !== before.y) ? c : null;
        });
        assert.ok(after, 'w/s should move player two within a tick');
...
test('a keeps player two moving left and reverse d is rejected [regression: bug A fixed]', async () => {
  ...
  try {
    ...
    assert.ok(after, 'a should move player two left');
    const reversed = headCell(dom, 'player2');
    press(dom, 'd');
    await sleep(160);
    const afterReverseAttempt = headCell(dom, 'player2');
    assert.ok(afterReverseAttempt.x < reversed.x, 'd should not reverse player two into itself');
  } finally {
    dom.window.close();
  }
});
...
test('wall collision shows game-over modal with final score; play-again resets [regression: bug B fixed]', async () => {
  ...
  try {
    ...
    assert.ok(Number.isFinite(Number(d.getElementById('finalScore').textContent)));
    assert.ok(Number(d.getElementById('finalLevel').textContent) >= 1);
